...
def detailed_operations_page(request, id):
    return render(request, 'operation_types_detailed.html', {
        'operations_to_perform': models.FacultyTypes.objects.filter(faculty_id=id)
    })
...
def operations_page(request):
    ...
    if query:
        ...
        filtered_data = {
            'operations_to_perform': models.FacultyTypes.objects.filter(Q(modeling_name__icontains=query))}

    else:
        filtered_data = {'operations_to_perform': models.FacultyTypes.objects.all()}
